...
export async function create(
    ...
    try
    {
        ...
        let res = await request.post(reqURL('/create-user'))
            .send({
...
export async function update(
    ...
    try
    {
        ...
        let res = await request.post(reqURL(`/update-user/${username}`))
            .send({ 
                username, password, provider, 
                familyName, givenName, middleName, emails, photos 
            })
...
export async function find(username)
{
    try
    {
        ...
        let res = await request.get(reqURL(`/find/${username}`))
            .set('Content-Type', 'application/json')
            .set('Acccept', 'application/json')
            .auth('them', 'D4ED43C0-8BD6-4FE2-B358-7C0E230D11EF');
...
export async function userPasswordCheck(username, password)
{
    try
    {
        ...
        let res = await request.post(reqURL(`/passwordCheck`))
...
export async function findOrCreate(profile)
{
    try
    {
        ...
        let res = await request.post(reqURL('/find-or-create'))
            .send({ 
                username: profile.id, 
...
export async function listUsers()
{
    try
    {
        ...
        let res = await request.get(reqURL('/list'))
            .set('Content-Type', 'application/json')
            .set('Acccept', 'application/json')
            .auth('them', 'D4ED43C0-8BD6-4FE2-B358-7C0E230D11EF');
