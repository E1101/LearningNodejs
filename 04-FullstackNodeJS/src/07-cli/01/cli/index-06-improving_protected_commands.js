...
// If our app knows that the user is not logged in and that they need to log in before
// using the edit product command, our app should not blindly run edit product and show
// the user a technical error with a stacktrace. The best thing to do is to run login for the user.
// ---
...
// ---
// Third, if a user enters an incorrect username or password, we know they would probably like to retry.
// We can't be certain, so the app should prompt for confirmation.
...
// will return without an authToken only if the
// user fails to log in (and declines to retry).
...
async function editProduct (opts) {
  ...
  // won't automatically send the API request if the user is not logged in.
...
// If the user is logged in, it will immediately return
// with the token. If the user is not logged in, it will
...
async function askRetry (error) {
  ...
  // If the error status code is 401, we know that the user entered
  // an invalid username/password combo
