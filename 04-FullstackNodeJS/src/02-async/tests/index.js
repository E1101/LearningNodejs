...
if (process.env.RUN_ALL) {
  tape('./01-passport-set-timeout.js', function (t) {
    testFile('./01-passport-set-timeout.js', function (err, out) {
      t.deepEqual(out, [
        '1 mississippi',
        '2 mississippi',
        '3 mississippi',
  ...
  tape('./02-configuration_management-creating_an_auth_module-validation-modulize-set-timeout-sync.js', function (t) {
    testFile('./02-configuration_management-creating_an_auth_module-validation-modulize-set-timeout-sync.js', function (err, out) {
...
tape('./03-health_check-session_sharing-persistence_with_JWTs-relationships-reading_query_parameters-read-file-callback.js', function (t) {
  testFile('./03-health_check-session_sharing-persistence_with_JWTs-relationships-reading_query_parameters-read-file-callback.js', function (err, out) {
    const [ file, count ] = out[0].split(': ')
    t.equal(file, '03-health_check-session_sharing-persistence_with_JWTs-relationships-reading_query_parameters-read-file-callback.js')
    t.ok(Number(count) > 100)
    t.end()
  })
})

tape('./04-adding_users-product_filtering-read-dir-callback.js', function (t) {
  testFile('./04-adding_users-product_filtering-read-dir-callback.js', function (err, out) {
    t.ok(out.length >= 24)
    t.ok(out.some(l => l === "  '03-health_check-session_sharing-persistence_with_JWTs-relationships-reading_query_parameters-read-file-callback.js',"))
    t.end()
  })
})

tape('./05-user_authorization-fetching_a_single_product-read-dir-callbacks-broken.js', function (t) {
  testFile('./05-user_authorization-fetching_a_single_product-read-dir-callbacks-broken.js', function (err, out) {
    t.equal(out[0], 'done!')
    t.ok(out.length >= 24)

    t.ok(out.some(function (l) {
      const [ file, count ] = l.split(': ')
      return (file === '03-health_check-session_sharing-persistence_with_JWTs-relationships-reading_query_parameters-read-file-callback.js') && Number(count) > 100
    }))
...
tape('./13-events-readline.js', function (t) {
  const child = fork('./13-events-readline.js', {silent: true })
...
tape('./15-events-readline-chat-send.js', function (t) {
  const child = fork('./15-events-readline-chat-send.js', {silent: true })
...
tape('./16-events-readline-chat-receive.js', function (t) {
  const child = fork('./16-events-readline-chat-receive.js', {silent: true })
...
tape('./17-events-create-emitter.js', function (t) {
  const child = fork('./17-events-create-emitter.js', {silent: true })
...
tape('./18-events-create-emitter-custom-events.js', function (t) {
  const child = fork('./18-events-create-emitter-custom-events.js', {silent: true })
...
tape('./23-streams-transform-shout.js', function (t) {
  testFile('./23-streams-transform-shout.js', function (err, out) {
